...
<a href="{{ env('FRONT_URL', 'http://localhost:3000') . '/email/confirm-change?token=' . $token }}">{{ __('emailChange.button') }}</a>
<p><small>{{ env('FRONT_URL', 'http://localhost:3000') . '/email/confirm-change?token=' . $token }}</small></p>
