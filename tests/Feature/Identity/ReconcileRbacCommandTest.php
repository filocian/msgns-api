...
describe('artisan rbac:reconcile', function () {
    ...
    it('seeds all 23 permissions into Spatie tables with stateful-api guard', function () {
        $this->artisan('rbac:reconcile')->assertExitCode(0);
    ...
    it('assigns all 23 permissions to developer role', function () {
        ...
        expect($role->permissions)->toHaveCount(23);
    });
    ...
    it('is idempotent — running twice does not change the state', function () {
        ...
        // Permissions count should be exactly 23 (no duplicates)
        $permCount = Permission::where('guard_name', 'stateful-api')->count();
        expect($permCount)->toBe(23);
    });
